...
To execute the primary strategy backtest engine against local historical datasets:
...
To spin up the web-based visualization interface and inspect the equity performance curves:
...
- **Zero-Latency Local Ingestion**: Built-in caching avoids redundant API throttling by reusing pre-loaded offline datasets instantly.
- **Anti-Fee Bleeding Logic**: Employs hyper-strict institutional filters to reduce high-frequency noise trading, dramatically lowering Binance exchange transaction fee overheads.
- **Asymmetric Risk Controls**: Structured dynamically to withstand sudden wick liquidations, achieving robust performance even during sudden market reversals.

---

🏁 **Disclaimer**: This project is built strictly for quantitative research and algorithmic simulation purposes. It does not constitute financial or direct investment advice.
